...
def _resolve_kimi_k3_npu_dcp(cfg: Any) -> dict:
    """Kimi-K3 DCP on Ascend: keep the ascend attention backend, default to
    replicated Q + a2a, and reject what the v1 NPU DCP path does not cover.
    ...
    if cfg.dcp_comm_backend == "fi_a2a":
        raise ValueError(
            "--dcp-comm-backend fi_a2a requires FlashInfer MNNVL and is not "
            "available on NPU; use 'a2a' or 'ag_rs'."
        )
...
def _kimi_k3_overrides(server_args: Any, hf_config: Any) -> dict:
    ...
    if cfg.dcp_size > 1 and get_platform().is_npu:
        return _resolve_kimi_k3_npu_dcp(cfg)
    if cfg.dcp_size > 1:
        overrides = {}
